...
<x-app-layout>
    ...
    <!--ヘッダー[END]-->
        ...
        <!-- バリデーションエラーの表示に使用-->
       <x-errors id="errors" class="bg-lime-500 rounded-lg">{{$errors}}</x-errors>
        <!-- バリデーションエラーの表示に使用-->
    ...
    <div class="flex-1 text-gray-700 text-left bg-blue-100 px-4 py-2 m-2">
        ...
        @if (count($books) > 0)
           @foreach ($books as $book)
               <x-collection books_id="{{ $book->books_id }}">{{ $book->item_name }}</x-collection>
           @endforeach
         @endif
    </div>
        <!--右側エリア[[END]-->
                ...
                @csrf
                  ...
                  <div class="flex flex-col">
                      <div class="text-gray-700 text-center px-4 py-2 m-2">
                             <x-button class="bg-lime-500 rounded-lg">送信</x-button>
                      </div>
                   </div>
            </form>
        </div>
    ...
    <!--右側エリア[START]-->
    <div class="flex-1 text-gray-700 text-left bg-blue-100 px-4 py-2 m-2">
        {{-- <x-collection>課題本1</x-collection>
        <x-collection>課題本2</x-collection>
        <x-collection>課題本3</x-collection> --}}
    </div>
    <!--右側エリア[[END]-->       
